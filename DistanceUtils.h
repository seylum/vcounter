...
template<typename T> static inline typename boost::enable_if_c<std::is_integral<T>::value,size_t>::type L1dist(T a, T b) {
    return (size_t)abs((int)a-b);
}

//! computes the L1 distance between two float values
template<typename T> static inline typename boost::enable_if_c<std::is_floating_point<T>::value,float>::type L1dist(T a, T b) {
...
template<size_t nChannels, typename T> static inline typename boost::enable_if_c<std::is_integral<T>::value,size_t>::type cdist(const T* curr, const T* bg) {
...
template<size_t nChannels, typename T> static inline typename boost::enable_if_c<std::is_floating_point<T>::value,float>::type cdist(const T* curr, const T* bg) {
    static_assert(nChannels>1,"cdist: requires more than one channel");
    float curr_sqr = 0;
    bool bSkip = true;
    for(size_t c=0; c<nChannels; ++c) {
...
template<size_t nChannels, typename T> static inline typename boost::enable_if_c<std::is_integral<T>::value,size_t>::type cmixdist(const T* curr, const T* bg) {
    return cmixdist(L1dist<nChannels>(curr,bg),cdist<nChannels>(curr,bg));
}

template<size_t nChannels, typename T> static inline typename boost::enable_if_c<std::is_floating_point<T>::value,float>::type cmixdist(const T* curr, const T* bg) {
    return cmixdist(L1dist<nChannels>(curr,bg),cdist<nChannels>(curr,bg));
}
